...
CREATE TABLE `userpreferences` (
  ...
  `PricePreference` varchar(255) DEFAULT NULL,
  `BrandPreference` text,
  `IngredientPreference` text,
  PRIMARY KEY (`PreferenceID`),
  KEY `User` (`User`),
  CONSTRAINT `userpreferences_ibfk_1` FOREIGN KEY (`User`) REFERENCES `users` (`Custusername`) ON DELETE CASCADE
